...
export function getSalaryForDate(history, dateStr) {
    ...
    // If dateStr is "2025-12-29T10:00:00", split gives "2025-12-29".
    // If dateStr is "2025-12-29", split gives "2025-12-29".
    const targetISO = String(dateStr).split('T')[0]

    // Sort history descending (newest first) by string comparison
    ...
    const entry = sorted.find(e => {
        const entryISO = String(e.date).split('T')[0]
        return entryISO <= targetISO
    })

    return entry ? { salary: entry.salary, hourlyRate: entry.hourlyRate } : null
}

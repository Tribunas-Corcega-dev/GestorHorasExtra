...
/**
 * Retrieves the salary information for a specific date from the history.
 * @param {array} history - Array of { date, salary, hourlyRate }
 * @param {string} dateStr - Target date (YYYY-MM-DD or ISO)
 * @returns {object|null} { salary, hourlyRate } or null if not found
 */
/**
 * Retrieves the salary information for a specific date from the history.
 ...
 * @returns {object|null} { salary, hourlyRate } or null if not found
 */
export function getSalaryForDate(history, dateStr) {
    if (!history || !Array.isArray(history) || history.length === 0 || !dateStr) {
        return null
    }

    const targetISO = String(dateStr).split('T')[0]

    // 1. Filter applicable entries (date <= target)
    //    We map to preserve original index for tie-breaking
    const applicable = history.map((h, i) => ({ ...h, originalIndex: i }))
        .filter(h => {
            if (!h.date) return false
            const hDate = String(h.date).split('T')[0]
            return hDate <= targetISO
        })

    if (applicable.length === 0) return null

    // 2. Sort to find the "Best" entry
    //    Priority 1: Date (Latest/Greatest is best)
    //    Priority 2: Original Insertion Order (Latest/Greatest index is best - tie breaker)
    applicable.sort((a, b) => {
        const da = String(a.date).split('T')[0]
        const db = String(b.date).split('T')[0]

        if (da > db) return -1 // a is newer date -> comes first
        if (da < db) return 1

        // Dates are equal, prefer the one added last (higher index)
        return b.originalIndex - a.originalIndex
    })

    return { salary: applicable[0].salary, hourlyRate: applicable[0].hourlyRate }
}
...
export function calculateEmployeeWorkValues(schedule, salary) {
    ...
    // Business rule: Monthly hours are projected on a 30-day month over a 6-day labor base.
    // Formula: (weeklyHours / 6) * 30 = weeklyHours * 5
 ...
 */
export function normalizeOvertimeType(dbType) {
    if (!dbType) return ""

    const normalized = dbType.trim().toLowerCase()
    const map = {
        "extra diurno": "extra_diurna",
        "trabajo extra diurno": "extra_diurna",
...
export function isExtraConcept(type) {
    return String(type || "").startsWith("extra_")
}

// Converts any legacy storage format into Mekano-style display percentage.
// Examples:
// extra: 0.25 -> 125, 25 -> 125, 125 -> 125
// surcharge: 0.35 -> 35, 35 -> 35, 110 -> 110
export function recargoToMekanoPercentage(recargoRaw, type) {
    const raw = Number(recargoRaw)
    if (!Number.isFinite(raw)) return 0

    if (isExtraConcept(type)) {
        if (raw <= 2) return (1 + raw) * 100
        if (raw < 100) return 100 + raw
        return raw
    }

    if (raw <= 2) return raw * 100
    return raw
}

// Converts stored recargo to a monetary factor applied over hourly rate.
// For extras: 125% => 1.25
// For surcharges: 35% => 0.35, 110% => 1.10
export function getRecargoPaymentFactor(recargoRaw, type) {
    const raw = Number(recargoRaw)
    if (!Number.isFinite(raw) || raw < 0) return 0

    if (isExtraConcept(type)) {
        if (raw <= 2) return 1 + raw
...
export function findRecargoConfig(surcharges, type) {
    if (!Array.isArray(surcharges) || !type) return null

    // Exact normalized match first
    const direct = surcharges.find((s) => normalizeOvertimeType(s?.tipo_hora_extra) === type)
    if (direct) {
        // Defensive fix for ambiguous "Trabajo nocturno" rows stored with different percentages.
        const rawType = String(direct?.tipo_hora_extra || "").toLowerCase()
        const rawValue = Number(direct?.recargo)
        const plainNocturno = /trabajo\s+nocturno/.test(rawType) && !/dominical|festivo/.test(rawType)

        if (plainNocturno) {
            if (type === "extra_nocturna" && Number.isFinite(rawValue) && rawValue <= 100) {
                const better = surcharges.find((s) => {
                    const t = String(s?.tipo_hora_extra || "").toLowerCase()
                    const v = Number(s?.recargo)
                    return /trabajo\s+nocturno/.test(t) && !/dominical|festivo/.test(t) && Number.isFinite(v) && v > 100
                })
                if (better) return better
            }
            if (type === "recargo_nocturno" && Number.isFinite(rawValue) && rawValue > 100) {
                const better = surcharges.find((s) => {
                    const t = String(s?.tipo_hora_extra || "").toLowerCase()
                    const v = Number(s?.recargo)
                    return /trabajo\s+nocturno/.test(t) && !/dominical|festivo/.test(t) && Number.isFinite(v) && v <= 100
    ...
    // Fallback for dirty legacy labels.
    if (type === "extra_nocturna") {
        return surcharges.find((s) => {
            const t = String(s?.tipo_hora_extra || "").toLowerCase()
            const v = Number(s?.recargo)
            return /trabajo\s+nocturno/.test(t) && !/dominical|festivo/.test(t) && Number.isFinite(v) && v > 100
        }) || null
...
/**
 * Calculates the monetary value of a specific overtime/surcharge duration.
 * @param {number} minutes - Duration in minutes
 * @param {number} hourlyRate - Employee's hourly rate
 * @param {number} recargoRaw - Stored recargo value
 * @param {string} type - Normalized concept type (e.g. extra_diurna, recargo_nocturno)
 * @returns {number} Calculated value
 */
export function calculateOvertimeValue(minutes, hourlyRate, recargoRaw, type) {
...
export function calculateTotalOvertimeValue(breakdown, hourlyRate, surcharges) {
    ...
    Object.entries(breakdown).forEach(([type, minutes]) => {
        if (minutes > 0) {
            const surcharge = findRecargoConfig(surcharges, type)
            const recargoRaw = surcharge ? surcharge.recargo : 0
            totalValue += calculateOvertimeValue(minutes, hourlyRate, recargoRaw, type)
        }
    })

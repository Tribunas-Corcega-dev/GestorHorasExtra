...
`Aprobaciones`/`Apelaciones` are currently removed from active sidebar/mobile navigation by design (deferred UI modules). Endpoint used for approvals remains: `GET /api/compensatorios/gestionar`.
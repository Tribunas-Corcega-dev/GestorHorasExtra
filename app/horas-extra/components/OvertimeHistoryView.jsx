...
import {
  calculateTotalOvertimeValue,
  formatToAmPm,
  getRecargoPaymentFactor,
  findRecargoConfig,
...
function getEmployeeSalaryForDate(empleado, dateValue) {
  ...
  if (
    Array.isArray(empleado.salary_history) &&
    empleado.salary_history.length > 0
  ) {
    const resolved = resolveEffectiveSalaryFromHistory(
      empleado.salary_history,
      dateValue,
    );
...
export function OvertimeHistoryView({ employeeId, showBackButton = true }) {
  ...
  const [showApprovalPreviewModal, setShowApprovalPreviewModal] =
    useState(false);
  const [approvalPeriod, setApprovalPeriod] = useState(null);
  const [showCompensationPreviewModal, setShowCompensationPreviewModal] =
    useState(false);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      mockPeriods.push({
        id: `${year}-${month}-0`,
        label: `Mes Completo ${monthName} ${year}`,
      });
      mockPeriods.push({
        id: `${year}-${month}-2`,
        label: `2ª Quincena ${monthName} ${year} (16-End)`,
      });
      mockPeriods.push({
        id: `${year}-${month}-1`,
        label: `1ª Quincena ${monthName} ${year} (01-15)`,
      });
    }
  ...
  async function fetchFixedSurcharges() {
    ...
    try {
      const res = await fetch(
        `/api/cierres/calcular?empleado_id=${employeeId}&periodo=${selectedPeriod}`,
      );
      if (res.ok) {
  ...
  const fetchBalanceSummary = async () => {
    try {
      const res = await fetch(
        `/api/compensatorios/resumen?empleado_id=${employeeId}`,
      );
      if (res.ok) {
        const data = await res.json();
  ...
  async function handleClosePeriod() {
    if (
      !confirm(
        "¿Estás seguro de cerrar esta quincena? Esto generará un registro oficial de nómina.",
      )
    )
      return;
  ...
  async function fetchData() {
    try {
      ...
      const balanceRes = await fetch(
        `/api/compensatorios/saldo?userId=${employeeId}`,
      );
      if (balanceRes.ok) {
  ...
  function getPeriodRange(periodId) {
    ...
    if (quincenaRaw === 1) {
      return {
        start: `${yearRaw}-${monthPadded}-01`,
        end: `${yearRaw}-${monthPadded}-15`,
      };
    }

    if (quincenaRaw === 2) {
      return {
        start: `${yearRaw}-${monthPadded}-16`,
        end: `${yearRaw}-${monthPadded}-${String(lastDay).padStart(2, "0")}`,
      };
    }
  ...
  const filteredSummary = (() => {
    ...
    filteredJornadas.forEach((jornada) => {
      if (jornada.horas_extra_hhmm) {
        ...
        const flatBreakdown =
          jornada.horas_extra_hhmm.flatBreakdown || breakdown;

        // Aggregate
        if (breakdown.overtime) {
          Object.entries(breakdown.overtime).forEach(
            ([k, v]) => (overtimeTotals[k] = (overtimeTotals[k] || 0) + v),
          );
        } else {
        ...
        if (breakdown.surcharges) {
          Object.entries(breakdown.surcharges).forEach(
            ([k, v]) => (surchargeTotals[k] = (surchargeTotals[k] || 0) + v),
          );
        } else {
          Object.entries(flatBreakdown).forEach(([k, v]) => {
  ...
  const mockFixedSurcharges = fixedSurchargesData
    ? {
        recargo_nocturno:
          fixedSurchargesData.fixedSurcharges.recargo_nocturno || 0,
        dominical_festivo:
          fixedSurchargesData.fixedSurcharges.dominical_festivo || 0,
        recargo_nocturno_festivo:
          fixedSurchargesData.fixedSurcharges.recargo_nocturno_festivo || 0,
                      ...
                      {empleado.valor_hora && (
                        <p>
                          ...
                          <span className="font-semibold text-foreground">
                            {formatCurrency(empleado.valor_hora)}
                          </span>
                        </p>
                      )}
                    </div>
                ...
                <div className="mt-4 bg-blue-50 dark:bg-blue-900/20 px-4 py-2 rounded-lg border border-blue-200 dark:border-blue-800 min-w-[200px]">
                  ...
                  {balanceData.saldo_pendiente > 0 && (
                    <p className="text-xs text-amber-600 dark:text-amber-500 mt-1 font-medium">
                      {formatMinutesToFloat(balanceData.saldo_pendiente)}{" "}
                      pendientes
                    </p>
                  )}
                  {isCoordinator && (
                    <div className="mt-3 text-center">
                      <button
                        onClick={() =>
                          router.push(`/horas-extra/${employeeId}/compensacion`)
                        }
      ...
      <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto mb-6">
        ...
        {canManageOvertime(user?.rol) && (
          <button
            ...
            title={
              selectedPeriod === "all"
                ? "Selecciona una quincena o mes"
                : "Abrir formato en vista previa"
            }
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        {canManageOvertime(user?.rol) && (
          <button
            ...
            title={
              selectedPeriod === "all"
                ? "Selecciona una quincena o mes"
                : "Ver formato de compensación en tiempo"
            }
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        {/* Close Period Button */}
        {["TALENTO_HUMANO", "ASISTENTE_GERENCIA"].includes(user?.rol) &&
          selectedPeriod !== "all" &&
          !closingRecord && (
            <button
      ...
      {closingRecord ? (
        <div className="bg-card border border-border rounded-lg p-6 shadow-sm mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-muted/50 p-3 rounded-md space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {formatMinutesToFloat(
                      closingRecord.recargos_fijos?.recargo_nocturno || 0,
                    )}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {formatMinutesToFloat(
                      closingRecord.recargos_fijos?.dominical_festivo || 0,
                    )}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {formatMinutesToFloat(
                      closingRecord.recargos_fijos?.recargo_nocturno_festivo ||
                        0,
                    )}
                  </span>
                </div>
            ...
            <div className="space-y-2">
              ...
              <div className="bg-muted/50 p-3 rounded-md space-y-2">
                {Object.entries(closingRecord.horas_extra_reportadas || {}).map(
                  ([key, minutes]) => {
                    if (minutes === 0) return null;
                    return (
                      <div key={key} className="flex justify-between text-sm">
                        <span className="truncate pr-2" title={LABELS[key]}>
                          {LABELS[key]}
                        </span>
                        <span className="font-medium">
            ...
            <div className="flex flex-col justify-center items-center bg-primary/5 rounded-lg p-4 border border-primary/10">
              ...
              <span className="text-3xl font-bold text-primary">
                {formatCurrency(closingRecord.valor_total)}
              </span>
              <p className="text-xs text-muted-foreground mt-2 text-center">
                Cierre generado el{" "}
                {new Date(closingRecord.created_at).toLocaleDateString()}
              </p>
            </div>
          </div>
        </div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div
              className={`space-y-4 ${selectedPeriod === "all" ? "opacity-50 grayscale" : ""}`}
            >
              ...
              <div className="bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900 rounded-lg p-4 shadow-sm">
                {selectedPeriod !== "all" ? (
                    ...
                    <>
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">
                            {formatMinutesToFloat(
                              mockFixedSurcharges.recargo_nocturno,
                            )}
                          </span>
                        </div>
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">
                            {formatMinutesToFloat(
                              mockFixedSurcharges.dominical_festivo,
                            )}
                          </span>
                        </div>
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">
                            {formatMinutesToFloat(
                              mockFixedSurcharges.recargo_nocturno_festivo,
                            )}
                          </span>
                        </div>
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center p-2 text-muted-foreground text-sm">
                    <p>
                      Selecciona una quincena para ver los recargos fijos
                      calculados.
                    </p>
                  </div>
                )}
            ...
            <div className="md:col-span-2 space-y-4">
              <h3 className="text-lg font-semibold text-foreground border-b border-border pb-2 flex items-center justify-between">
                ...
                <span className="text-xs font-normal text-muted-foreground">
                  Basado en {filteredJornadas.length} registros
                </span>
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-card border border-border rounded-lg p-4 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {Object.entries(filteredSummary.overtimeTotals).map(
                      ([key, minutes]) => {
                        if (minutes === 0) return null;
                    ...
                    {Object.values(filteredSummary.overtimeTotals).every(
                      (v) => v === 0,
                    ) && (
                ...
                <div className="bg-card border border-border rounded-lg p-4 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {Object.entries(filteredSummary.surchargeTotals).map(
                      ([key, minutes]) => {
                        if (minutes === 0) return null;
                    ...
                    {Object.values(filteredSummary.surchargeTotals).every(
                      (v) => v === 0,
                    ) && (
                      <p className="text-sm text-muted-foreground italic">
                        - Sin registros -
          ...
          <div className="flex justify-center mb-8">
            ...
            <div className="w-full md:w-2/3 bg-blue-50 border border-blue-100 dark:bg-blue-900/10 dark:border-blue-800 rounded-lg p-6 shadow-sm flex flex-col items-center justify-center text-center">
              <h3 className="text-sm font-bold text-blue-800 dark:text-blue-300 uppercase tracking-wider mb-2">
                Total Solo Horas Extra
              </h3>
              <div className="text-2xl font-bold text-blue-700 dark:text-blue-400">
                {formatCurrency(
                  Object.values(filteredSummary.overtimeValues).reduce(
                    (a, b) => a + b,
                    0,
                  ),
                )}
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-border">
                {filteredJornadas.map((jornada) => {
                  ...
                  const dayName =
                    jornada.jornada_base_calcular?.dayOfWeek || "";
                  const overtimeFormatted =
                    jornada.horas_extra_hhmm?.formatted || "-";
                  const overtimeMinutes =
                    jornada.horas_extra_hhmm?.minutes || 0;
                  ...
                  const hasOvertime = Object.values(overtimeBreakdown).some(
                    (v) => v > 0,
                  );
                  const hasSurcharges = Object.values(surchargeBreakdown).some(
                    (v) => v > 0,
                  );

                  // Determine hourly rate
                  let hourlyRate = 0;
                  const historySalary = getEmployeeSalaryForDate(
                    empleado,
                    jornada.fecha,
                  );
                  ...
                  if (hourlyRate > 0 && recargos.length > 0) {
                    dayValue = calculateTotalOvertimeValue(
                      flatBreakdown,
                      hourlyRate,
                      recargos,
                    );
                  ...
                  return (
                    ...
                    >
                        ...
                        {jornada.horas_para_bolsa_minutos > 0 && (
                          <div className="mt-1">
                            <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300">
                              Compensación:{" "}
                              {formatMinutesToFloat(
                                jornada.horas_para_bolsa_minutos,
                              )}
                            </span>
                          </div>
                        )}
                        ...
                        <div className="flex flex-col gap-1">
                          {schedule.morning.enabled && (
                            <span className="text-xs">
                              M: {formatToAmPm(schedule.morning.start)}-
                              {formatToAmPm(schedule.morning.end)}
                            </span>
                          )}
                          {schedule.afternoon.enabled && (
                            <span className="text-xs">
                              T: {formatToAmPm(schedule.afternoon.start)}-
                              {formatToAmPm(schedule.afternoon.end)}
                            </span>
                          )}
                          {!schedule.morning.enabled &&
                            !schedule.afternoon.enabled && (
                      ...
                      <td className="px-4 py-3 text-sm">
                        {hasOvertime ? (
                          <div className="flex flex-wrap gap-1">
                            {Object.entries(overtimeBreakdown).map(
                              ([key, minutes]) => {
                                if (minutes <= 0) return null;
                                return (
                                  <span
                                    key={key}
                                    className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-200 dark:border-blue-800 mr-1 mb-1"
                                    title={LABELS[key]}
                                  >
                                    {LABELS[key]}:{" "}
                                    {formatMinutesToFloat(minutes)}
                      ...
                      <td className="px-4 py-3 text-sm">
                        {hasSurcharges ? (
                          <div className="flex flex-wrap gap-1">
                            {Object.entries(surchargeBreakdown).map(
                              ([key, minutes]) => {
                                if (minutes <= 0) return null;
                                return (
                                  <span
                                    key={key}
                                    className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400 border border-orange-200 dark:border-orange-800 mr-1 mb-1"
                                    title={LABELS[key]}
                                  >
                                    {LABELS[key]}:{" "}
                                    {formatMinutesToFloat(minutes)}
                      ...
                      <td className="px-4 py-3 text-sm font-bold text-green-600 dark:text-green-400 text-right whitespace-nowrap">
                        {["SOLICITADO", "APROBADO"].includes(
                          jornada.estado_compensacion,
                        ) ? (
          ...
          <div className="bg-card border border-border rounded-xl shadow-2xl max-w-lg w-full overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium text-foreground">
                    {formatDateForDisplay(selectedJornada.fecha)}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">
              ...
              <div>
                ...
                {(() => {
                  const breakdown =
                    selectedJornada.horas_extra_hhmm?.breakdown || {};
                  let flatBreakdown = {};
                  ...
                  const hasItems = Object.values(flatBreakdown).some(
                    (v) => v > 0,
                  );
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-sm font-bold pt-2 border-t border-border">
                        ...
                        <span className="text-primary">
                          {selectedJornada.horas_extra_hhmm.formatted}
                        </span>
                      </div>
                    </div>
                  );
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="h-6 w-6 rounded-full bg-primary/20 flex items-center justify-center text-xs font-bold text-primary">
                      {(
                        selectedJornada.registrador?.nombre ||
                        selectedJornada.registrador?.username ||
                        "?"
                      )
                        .charAt(0)
                        .toUpperCase()}
                  ...
                  {selectedJornada.aprobador ? (
                    <div className="flex items-center gap-2">
                      <div className="h-6 w-6 rounded-full bg-green-100 text-green-700 flex items-center justify-center text-xs font-bold">
                        {(
                          selectedJornada.aprobador.nombre ||
                          selectedJornada.aprobador.username ||
                          "?"
                        )
                          .charAt(0)
                          .toUpperCase()}
